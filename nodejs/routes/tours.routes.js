...
module.exports = function(server){
    server.get('/tours', function(req, res){
        db.query("SELECT tours.id, tours.name as namet, tours.price, tours.sale_price, tours.image, category.name as namec, tours.status, tours.description FROM tours JOIN category ON tours.category_id = category.id", 
        function(err, data){
            let message = null;
            if(err){
    ...
    server.post('/tours', function(req, res){
        ...
        let SQL2 = "SELECT tours.id, tours.name as namet, tours.price, tours.sale_price, tours.image, category.name as namec, tours.status, tours.description FROM tours JOIN category ON tours.category_id = category.id WHERE tours.name like ?";
